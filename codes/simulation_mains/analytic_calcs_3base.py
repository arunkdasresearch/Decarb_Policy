...
delay_inds = np.arange(0, 7, 1)  # sector indices for delay
...
for delay_ind in delay_inds:
    ...
    for delay_amount in delay_amounts:
        ...
        # in the challenged sector
        B_p_star = delay_amount * params_w_bstar[delay_ind, 1]

        # store in big list for saving later
        B_ps[delay_sec_count, delay_count] = B_p_star

        # set t0 in the delayed sector to delay_amount
        tmp_params_chal = params_w_bstar.copy()
        tmp_params_chal[delay_ind, 3] = delay_amount  # set t0 = delay_amount

        # make temporary parameter lists
        tmp_params_chal_forfunc = [tmp_params_chal[delay_ind, :-1]]
        tmp_params_nochal = np.delete(params_w_bstar, delay_ind, axis=0)[:,
                                                                         :-1]

        # set caps
        # challenged cap: optimal allocation + premium - t0 * abar
        chal_cap = tmp_params_chal[delay_ind, -1] + B_p_star\
            - params_w_bstar[delay_ind][1] * delay_amount

        # non challenged cap: optimal allocation - premium - t0 * abar
        # NOTE: in this case, the last bit is zero so i don't include it here
        nochal_cap = np.sum(np.delete(params_w_bstar, delay_ind, axis=0).T[-1])\
                            - B_p_star
        ...
        opt_e = fsolve(get_decarb_time_and_price,
                       x0=guess,
                       args=[df_glob['r'], nochal_cap, tmp_params_nochal])
...
if save_output:
    ...
    cwd = os.getcwd()
    path = ''.join([cwd, '/data/output/', cal, '_base3_analytics.nc'])
